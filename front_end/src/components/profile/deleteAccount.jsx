...
const DeleteAccount = (props) => {
  ...
  const deleteAccount = () => {
    axios
      .post("http://localhost:8080/api/users/deleteUser", {
        email: currUser.email,
      })
      .then(function (response) {
        if (response.data.status === Constants.SUCCESS) {
          localStorage.setItem(Constants.USER, null);
      ...
      >
        ...
        <Modal.Body>
          <Text id="modal-title" size={15}>
            You will be removed from all of your family groups, but your events
            will still remain.
          </Text>
          <Text id="modal-title" size={15}>
            If you would like to delete your events, please do so manually
