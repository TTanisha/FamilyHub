...
const UserProfile = (props) => {
  ...
  const GetMemberData = () => {
    axios
      .post("http://localhost:8080/api/users/getUserById", { id: props.userId })
  ...
  const submitUpdateUser = (props) => {
    axios
      .post("http://localhost:8080/api/users/updateUser", {
        email: email,
        firstName: firstName,
        lastName: lastName,
      ...
      .then(function (response) {
        if (response.data.status === Constants.SUCCESS) {
          ...
          updateLocalStorage(JSON.parse(localStorage.getItem(Constants.USER)));
          setEditing(false);
  ...
  useEffect(() => {
    if (props.currUser == false) {
      ...
      currUser = JSON.parse(localStorage.getItem(Constants.USER));
              ...
              <>
                <Grid xs={3}>
                  <Button
                    ...
                    onPress={() => {
                      setEditing(true);
                      setClickable({ pointerEvents: "auto" });
                    }}
                  >
                    Edit
                  </Button>
