...
function setCalendarTitle(view) {
  ...
  //change calendar title string based on view
  if (view == Constants.MONTHLY) {
    string = moment(
      tuiCalendar.getDate().toString(),
      "ddd MMM DD YYYY HH:mm:ss"
    ).format("MMMM YYYY");
  } else if (view == Constants.DAILY) {
    string = moment(
      tuiCalendar.getDate().toString(),
      "ddd MMM DD YYYY HH:mm:ss"
    ).format("ddd MMMM DD YYYY");
  } else if (view == Constants.WEEKLY) {
    let weekStart = moment(
      tuiCalendar.getDateRangeStart().toString(),
      "ddd MMM DD YYYY HH:mm:ss"
    ).format("ddd MMMM DD YYYY");

    let weekEnd = moment(
      tuiCalendar.getDateRangeEnd().toString(),
      "ddd MMM DD YYYY HH:mm:ss"
    ).format("ddd MMMM DD YYYY");
...
const Calendar = () => {
  ...
  const [selectedView, setSelectedView] = useState(Constants.MONTHLY);
  const [pickerDate, setPickerDate] = useState();
  const [selectedFilters, setSelectedFilters] = useState(currUser.groups);

  useEffect(() => {
    createCalendar();
    setCalendarTitle(Constants.MONTHLY);
  ...
  function handleSetCalendarTitle() {
    if (selectedView != Constants.MONTHLY) {
  ...
  useEffect(() => {
    if (selectedView != Constants.MONTHLY) {
      if (selectedView.currentKey == Constants.MONTHLY) {
        tuiCalendar.changeView(Constants.MONTH);
      } else if (selectedView.currentKey == Constants.DAILY) {
        tuiCalendar.changeView(Constants.DAY);
        tuiCalendar.setOptions({
          week: {
            taskView: false,
          },
        });
      } else if (selectedView.currentKey == Constants.WEEKLY) {
        tuiCalendar.changeView(Constants.WEEK);
        tuiCalendar.setOptions({
          week: {
            taskView: false,
          },
  ...
  useEffect(() => {
    if (data) {
      usersEvents?.map((event) => {
        ...
        setTUICalendarEvents((tuiCalendarEvents) => [
          ...tuiCalendarEvents,
          tuiEvent,
        ]);
      });
  ...
  async function getFamilyGroupName(groupId) {
    let res = Promise.resolve(
      await axios
        .post("http://localhost:8080/api/familyGroups/getFamilyGroup", {
          groupId: groupId,
        })
        ...
        .catch(function (error) {
          if (error.message != Constants.CANCELED) {
            console.log(error);
          }
        })
    );
  ...
  async function getUsersEvents(controller) {
    const promises = await Promise.all(
      currUser.groups?.map(async (id) => {
        return await axios
          .post(
            "http://localhost:8080/api/familyGroups/getFamilyGroupEvents",
            {
              groupId: id,
            },
      ...
      <div className="filters-container">
        <Grid.Container justify="center" direction="column">
          ...
          <Dropdown>
            ...
            >
              <Dropdown.Item key={Constants.MONTHLY}>
                {Constants.MONTHLY}
              </Dropdown.Item>
              <Dropdown.Item key={Constants.WEEKLY}>
                {Constants.WEEKLY}
              </Dropdown.Item>
              <Dropdown.Item key={Constants.DAILY}>
                {Constants.DAILY}
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
